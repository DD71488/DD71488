  ...
  <div class="batch-daily-tasks">
    <div class="main-layout">
      ...
      <div class="left-column">
          ...
          <div style="display: flex; align-items: center; gap: 16px">
            ...
            >
              ...
              <div style="display: flex; gap: 8px">
                <a-button type="primary" size="small" @click="openTaskModal">
                  新增定时任务
                </a-button>
                <a-button size="small" @click="showTasksModal = true">
                  查看定时任务
                </a-button>
                ...
                >
                  <a-button size="small">导入配置</a-button>
                </a-upload>
              </div>
            </div>
          </div>
        ...
        <!-- Token Selection -->
        <a-card title="账号列表" class="token-list-card">
          ...
          <a-space vertical>
            ...
            <a-checkbox-group v-model="selectedTokens">
              <a-grid
                :x-gap="12"
                :y-gap="8"
                :cols="batchSettings.tokenListColumns"
              >
                <a-grid-item v-for="token in sortedTokens" :key="token.id">
                  <div class="token-row">
                    <a-checkbox
                      :value="token.id"
                      :label="token.name"
        ...
        <!-- Batch Functions -->
        <a-card title="批量功能列表" style="margin-top: 16px">
          <a-tabs type="line" animated>
            ...
            <a-tab-pane key="dungeon" title="副本">
              <a-space>
                <a-button
                  size="small"
                  @click="climbTower"
            ...
            <a-tab-pane key="weirdTower" title="怪异塔">
              <a-space>
                <a-button
                  size="small"
            ...
            <a-tab-pane key="resource" title="资源">
              <a-space>
                <a-button
                  size="small"
            ...
            <a-tab-pane key="legacy" title="功法">
              <a-space>
                <a-button
                  size="small"
            ...
            <a-tab-pane key="monthly" title="月度">
              <a-space>
                <a-button
                  size="small"
          ...
          <template #header>
            <div class="custom-card-header">
              ...
              <div class="log-header-controls">
                <a-checkbox v-model="autoScrollLog" size="small">
                  自动滚动
                </a-checkbox>
                <a-checkbox v-model="filterErrorsOnly" size="small">
                  只看错误
                </a-checkbox>
                <a-tag v-if="errorCount > 0" type="error" size="small">
                  {{ errorCount }} 个错误
                </a-tag>
                <a-button size="small" @click="clearLogs"> 清空日志 </a-button>
                <a-button size="small" @click="copyLogs"> 复制日志 </a-button>
              </div>
            </div>
          </template>
          <a-progress
      ...
      <div class="settings-content">
        <div class="settings-grid">
          <div class="setting-item">
            ...
            <a-select
              v-model="currentSettings.arenaFormation"
          ...
          <div class="setting-item">
            ...
            <a-select
              v-model="currentSettings.towerFormation"
              :options="formationOptions"
              size="small"
            />
          </div>
          <div class="setting-item">
            ...
            <a-select
              v-model="currentSettings.bossFormation"
              :options="formationOptions"
              size="small"
            />
          </div>
          <div class="setting-item">
            ...
            <a-select
              v-model="currentSettings.bossTimes"
          ...
          <div class="setting-switches">
            <div class="switch-row">
              ...
              ><a-switch v-model="currentSettings.claimBottle" />
            </div>
            <div class="switch-row">
              <span class="switch-label">领挂机</span
              ><a-switch v-model="currentSettings.claimHangUp" />
            </div>
            <div class="switch-row">
              <span class="switch-label">竞技场</span
              ><a-switch v-model="currentSettings.arenaEnable" />
            </div>
            <div class="switch-row">
              <span class="switch-label">开宝箱</span
              ><a-switch v-model="currentSettings.openBox" />
            </div>
            <div class="switch-row">
              <span class="switch-label">领取邮件奖励</span
              ><a-switch v-model="currentSettings.claimEmail" />
            </div>
            <div class="switch-row">
              <span class="switch-label">黑市购买物品</span
              ><a-switch v-model="currentSettings.blackMarketPurchase" />
            </div>
            <div class="switch-row">
              <span class="switch-label">付费招募</span
              ><a-switch v-model="currentSettings.payRecruit" />
            </div>
          </div>
        </div>
    ...
    <!-- Task Template Modal -->
    <a-modal
      v-model:visible="showTaskTemplateModal"
      preset="card"
      ...
      <div class="settings-content">
        <div class="settings-grid">
          ...
          <div class="setting-item">
            <label class="setting-label">竞技场阵容</label>
            <a-select
              v-model="currentTemplate.arenaFormation"
              :options="formationOptions"
              size="small"
            />
          </div>
          <div class="setting-item">
            <label class="setting-label">爬塔阵容</label>
            <a-select
              v-model="currentTemplate.towerFormation"
              :options="formationOptions"
              size="small"
            />
          </div>
          <div class="setting-item">
            <label class="setting-label">BOSS阵容</label>
            <a-select
              v-model="currentTemplate.bossFormation"
              :options="formationOptions"
              size="small"
            />
          </div>
          <div class="setting-item">
            <label class="setting-label">BOSS次数</label>
            <a-select
              v-model="currentTemplate.bossTimes"
              :options="bossTimesOptions"
              size="small"
            />
          </div>
          <div class="setting-switches">
            <div class="switch-row">
              <span class="switch-label">领罐子</span
              ><a-switch v-model="currentTemplate.claimBottle" />
            </div>
            <div class="switch-row">
              <span class="switch-label">领挂机</span
              ><a-switch v-model="currentTemplate.claimHangUp" />
            </div>
            <div class="switch-row">
              <span class="switch-label">竞技场</span
              ><a-switch v-model="currentTemplate.arenaEnable" />
            </div>
            <div class="switch-row">
              <span class="switch-label">开宝箱</span
              ><a-switch v-model="currentTemplate.openBox" />
            </div>
            <div class="switch-row">
              <span class="switch-label">领取邮件奖励</span
              ><a-switch v-model="currentTemplate.claimEmail" />
            </div>
            <div class="switch-row">
              <span class="switch-label">黑市购买物品</span
              ><a-switch v-model="currentTemplate.blackMarketPurchase" />
            </div>
            <div class="switch-row">
              <span class="switch-label">付费招募</span
              ><a-switch v-model="currentTemplate.payRecruit" />
            </div>
          </div>
        </div>
        <div class="modal-actions" style="margin-top: 20px; text-align: right">
          <a-button
            ...
            >取消</a-button
          >
          <a-button @click="saveTaskTemplate" type="primary">保存模板</a-button>
        </div>
      </div>
    </a-modal>
    ...
    <a-modal
      v-model:visible="showApplyTemplateModal"
      preset="card"
      title="应用任务模板"
      ...
      <div class="settings-content">
        <div class="settings-grid">
          ...
          <div class="setting-item">
            ...
            <a-checkbox-group
              v-model="selectedTokensForApply"
              style="margin-top: 8px"
            >
              <a-grid :cols="2" :x-gap="12" :y-gap="8">
                <a-grid-item v-for="token in sortedTokens" :key="token.id">
                  <a-checkbox :value="token.id">{{ token.name }}</a-checkbox>
                </a-grid-item>
              </a-grid>
            </a-checkbox-group>
          </div>
        ...
        <div class="modal-actions" style="margin-top: 20px; text-align: right">
          <a-button @click="showApplyTemplateModal = false">取消</a-button>
          <a-button
            @click="applyTemplate"
            type="success"
            :disabled="
    ...
    >
      <div class="settings-content">
        ...
        >
          <div>
            <a-button type="primary" @click="openNewTemplateModal"
              >新增模板</a-button
            >
            <a-button
          ...
          <a-input placeholder="搜索模板" size="small" style="width: 200px" />
        </div>
        ...
        >
          ...
          >
            ...
            >
              ...
              <div style="display: flex; gap: 8px">
                <a-button size="small" @click="openEditTemplateModal(template)"
                  >编辑</a-button
                >
                <a-button
                  size="small"
        ...
        <!-- Actions -->
        <div class="modal-actions" style="margin-top: 20px; text-align: right">
          <a-button @click="showTemplateManagerModal = false">关闭</a-button>
        </div>
      </div>
    </a-modal>
    ...
    <a-modal
      v-model:visible="showAccountTemplateModal"
      preset="card"
      title="账号模板引用查看"
      style="width: 90%; max-width: 800px"
    >
      <div class="settings-content">
        ...
        >
          ...
          <div style="display: flex; gap: 8px; align-items: center">
            ...
            <a-select
              v-model="selectedTemplateForFilter"
              :options="taskTemplates"
              label-field="name"
              value-field="id"
              placeholder="全部模板"
        ...
        <!-- Actions -->
        <div class="modal-actions" style="margin-top: 20px; text-align: right">
          <a-button @click="showAccountTemplateModal = false">关闭</a-button>
        </div>
      </div>
    </a-modal>
    ...
    <!-- Legacy Gift Modal -->
    <a-modal
      v-model:visible="showLegacyGiftModal"
      preset="card"
      title="批量功法残卷赠送"
      style="width: 90%; max-width: 600px"
    >
    ...
    <a-modal
      v-model:visible="showHelperModal"
      preset="card"
      ...
      <div class="settings-content">
        <div class="settings-grid">
          <div class="setting-item" v-if="helperType === 'box'">
            ...
            <a-select
              v-model="helperSettings.boxType"
          ...
          <div class="setting-item" v-if="helperType === 'fish'">
            ...
            <a-select
              v-model="helperSettings.fishType"
          ...
          <div class="setting-item" v-if="helperType === 'pointsBox'">
            ...
            <a-input-number
              v-model="helperSettings.targetPoints"
              :min="1"
              :max="1000000"
              :step="100"
              size="small"
        ...
        <div class="modal-actions" style="margin-top: 20px; text-align: right">
          <a-button @click="showHelperModal = false" style="margin-right: 12px"
            >取消</a-button
          >
          <a-button type="primary" @click="executeHelper">开始执行</a-button>
        </div>
      </div>
    </a-modal>
    ...
    <!-- Dream Buy Modal -->
    <a-modal
      v-model:visible="showDreamBuyModal"
      preset="card"
      title="梦境商品购买配置"
      style="width: 90%; max-width: 600px"
    >
      <div class="settings-content">
        <div class="settings-grid">
          ...
          <div style="display: flex; gap: 12px; margin-bottom: 12px">
            <a-button size="small" type="warning" @click="selectGoldItems">
              一键勾选金币商品
            </a-button>
            <a-button size="small" @click="selectAllItems"> 全选所有 </a-button>
            <a-button size="small" @click="clearAllItems"> 清空选择 </a-button>
          </div>
          ...
          >
            ...
            <a-grid :cols="3" :x-gap="12" :y-gap="8">
              <a-grid-item v-for="(item, index) in merchant.items" :key="index">
                <a-checkbox
                  :value="`${id}-${index}`"
        ...
        <div class="modal-actions" style="margin-top: 20px; text-align: right">
          <a-button
            ...
            >取消</a-button
          >
          <a-button type="primary" @click="saveDreamBuyConfig"
            >保存配置</a-button
          >
        </div>
      </div>
    ...
    <!-- Tasks List Modal -->
    <a-modal
      v-model:visible="showTasksModal"
      preset="card"
      title="定时任务列表"
      style="width: 90%; max-width: 800px"
    >
      <div class="tasks-list" style="max-height: 600px; overflow-y: auto">
        ...
        >
          ...
          <div style="display: flex; gap: 8px">
            <a-button size="tiny" @click="editTask(task)"> 编辑 </a-button>
            <a-button size="tiny" type="error" @click="deleteTask(task.id)">
              删除
            </a-button>
            <a-button
              size="tiny"
      ...
      <div class="settings-content">
        <div class="settings-grid">
          ...
          <div class="setting-item" v-if="taskForm.runType === 'daily'">
            ...
            <a-time-picker v-model="taskForm.runTime" format="HH:mm" />
          </div>
          <div class="setting-item" v-if="taskForm.runType === 'cron'">
            ...
            <div class="cron-parser" v-if="taskForm.cronExpression">
              <div v-if="cronValidation.valid" class="cron-validation success">
                <a-typography type="success">✓ {{ cronValidation.message }}</a-typography>
              </div>
              <div v-else class="cron-validation error">
                <a-typography type="error">✗ {{ cronValidation.message }}</a-typography>
              </div>
          ...
          <div class="setting-item">
            ...
            >
              <label class="setting-label">选择账号</label>
              <a-space size="small">
                <a-button size="small" @click="selectAllTokens">
                  全选
                </a-button>
                <a-button size="small" @click="deselectAllTokens">
                  全不选
                </a-button>
              </a-space>
            </div>
            ...
            <a-checkbox-group v-model="taskForm.selectedTokens">
              <a-grid :cols="2" :x-gap="12" :y-gap="8">
                <a-grid-item v-for="token in sortedTokens" :key="token.id">
                  <a-checkbox :value="token.id">{{ token.name }}</a-checkbox>
                </a-grid-item>
          ...
          <div class="setting-item">
            ...
            >
              ...
              <a-space size="small">
                <a-button size="small" @click="selectAllTasks"> 全选 </a-button>
                <a-button size="small" @click="deselectAllTasks">
                  全不选
                </a-button>
              </a-space>
            </div>

            <a-checkbox-group v-model="taskForm.selectedTasks">
              <a-tabs
                type="line"
                animated
                size="small"
        ...
        <div class="modal-actions" style="margin-top: 20px; text-align: right">
          <a-button @click="showTaskModal = false" style="margin-right: 12px"
            >取消</a-button
          >
    ...
    <a-modal
      v-model:visible="showBatchSettingsModal"
      preset="card"
      title="任务设置"
      style="width: 90%; max-width: 700px"
    >
      <div class="settings-content">
        <a-grid :cols="2" :x-gap="24">
          <!-- 左列：批量操作设置 -->
          <a-grid-item>
            <a-divider title-placement="left" style="margin: 1px 0 8px 0"
              >批量操作设置</a-divider
            >
            <div class="settings-grid">
              <div
              ...
              >
                ...
                <a-input-number
                  v-model="batchSettings.boxCount"
                  :min="10"
                  :max="10000"
                  :step="10"
                  size="small"
              ...
              >
                ...
                <a-input-number
                  v-model="batchSettings.fishCount"
                  :min="10"
                  :max="10000"
                  :step="10"
                  size="small"
              ...
              >
                ...
                <a-input-number
                  v-model="batchSettings.recruitCount"
                  :min="10"
                  :max="10000"
                  :step="10"
                  size="small"
              ...
              >
                ...
                <a-select
                  v-model="batchSettings.defaultBoxType"
                  :options="boxTypeOptions"
                  size="small"
                  style="width: 100px"
                />
              ...
              >
                ...
                <a-select
                  v-model="batchSettings.defaultFishType"
                  :options="fishTypeOptions"
                  size="small"
                  style="width: 100px"
                />
              ...
              >
                ...
                <a-input-number
                  v-model="batchSettings.targetBoxPoints"
                  :min="1"
                  :max="1000000"
                  :step="100"
                  size="small"
              ...
              >
                ...
                <a-button size="small" @click="openDreamBuyModal"
                  >点击配置</a-button
                >
              </div>
            </div>
            <a-divider title-placement="left" style="margin: 12px 0 8px 0"
              >智能发车条件设置(0为不限制)</a-divider
            >
            <div class="settings-grid">
              <div
                class="setting-item"
              ...
              >
                ...
                <a-select
                  v-model="batchSettings.carMinColor"
                  :options="[
                    { label: '绿·普通', value: 1 },
                    { label: '蓝·稀有', value: 2 },
                    { label: '紫·史诗', value: 3 },
              ...
              >
                ...
                <a-switch
                  v-model="batchSettings.useGoldRefreshFallback"
                />
              </div>
            </div>
            <div
            ...
            >
              ...
              >
                ...
                <a-switch
                  v-model="batchSettings.smartDepartureMatchAll"
                />
              </div>
              <div
                class="setting-item"
              ...
              >
                ...
                <a-input-number
                  v-model="batchSettings.smartDepartureGoldThreshold"
                  :min="0"
                  :step="100"
                  size="small"
                  style="width: 100px"
              ...
              >
                ...
                <a-input-number
                  v-model="batchSettings.smartDepartureRecruitThreshold"
                  :min="0"
                  :step="10"
                  size="small"
                  style="width: 100px"
              ...
              >
                ...
                <a-input-number
                  v-model="batchSettings.smartDepartureJadeThreshold"
                  :min="0"
                  :step="100"
                  size="small"
                  style="width: 100px"
              ...
              >
                ...
                <a-input-number
                  v-model="batchSettings.smartDepartureTicketThreshold"
                  :min="0"
                  :step="1"
                  size="small"
                  style="width: 100px"
            ...
            <div class="settings-grid">
              ...
              >
                <label class="setting-label">接收者ID</label>
                <a-input-number
                  v-model="batchSettings.receiverId"
                  placeholder="ID"
                  size="small"
                  style="width: 100px"
                  :show-button="false"
              ...
              >
                ...
                <a-input
                  v-model="batchSettings.password"
                  type="password"
                  placeholder="密码"
                  size="small"
                  style="width: 100px"
          ...
          <a-grid-item>
            ...
            <div class="settings-grid">
              ...
              >
                ...
                <a-input-number
                  v-model="batchSettings.commandDelay"
                  :min="100"
                  :max="2000"
                  :step="100"
                  size="small"
              ...
              >
                ...
                <a-input-number
                  v-model="batchSettings.taskDelay"
                  :min="100"
                  :max="2000"
                  :step="100"
                  size="small"
              ...
              >
                ...
                <a-input-number
                  v-model="batchSettings.actionDelay"
                  :min="100"
                  :max="2000"
                  :step="100"
                  size="small"
              ...
              >
                ...
                <a-input-number
                  v-model="batchSettings.battleDelay"
                  :min="100"
                  :max="2000"
                  :step="100"
                  size="small"
              ...
              >
                ...
                <a-input-number
                  v-model="batchSettings.refreshDelay"
                  :min="500"
                  :max="3000"
                  :step="100"
                  size="small"
              ...
              >
                ...
                <a-input-number
                  v-model="batchSettings.longDelay"
                  :min="1000"
                  :max="10000"
                  :step="500"
                  size="small"
            ...
            <div class="settings-grid">
              ...
              >
                ...
                <a-input-number
                  v-model="batchSettings.maxActive"
                  :min="1"
                  :max="20"
                  :step="1"
                  size="small"
              ...
              >
                ...
                <a-input-number
                  v-model="batchSettings.connectionTimeout"
                  :min="1000"
                  :max="30000"
                  :step="1000"
                  size="small"
              ...
              >
                ...
                <a-input-number
                  v-model="batchSettings.reconnectDelay"
                  :min="100"
                  :max="5000"
                  :step="100"
                  size="small"
            ...
            <div class="settings-grid">
              ...
              >
                ...
                <a-input-number
                  v-model="batchSettings.tokenListColumns"
                  :min="1"
                  :max="10"
                  :step="1"
                  size="small"
              ...
              >
                ...
                <a-input-number
                  v-model="batchSettings.maxLogEntries"
                  :min="100"
                  :max="5000"
                  :step="100"
                  size="small"
              ...
              >
                ...
                <a-switch v-model="batchSettings.enableRefresh" />
              </div>
              <div
                class="setting-item"
                style="
              ...
              >
                ...
                <a-input-number
                  v-model="batchSettings.refreshInterval"
                  :min="10"
                  :max="1440"
                  :step="30"
                  size="small"
        ...
        <div class="modal-actions" style="margin-top: 20px; text-align: right">
          <a-button
            ...
            >取消</a-button
          >
          <a-button type="primary" @click="saveBatchSettings"
            >保存设置</a-button
          >
        </div>
      </div>
    ...
    <!-- War Guess Modal -->
    <a-modal
      v-model:visible="showWarGuessModal"
      preset="card"
      title="月赛助威"
      style="width: 90%; max-width: 800px"
    >
      <div class="settings-content">
        <div class="settings-grid" style="display: block">
          ...
          >
            ...
            <a-button @click="fetchWarGuessRank" :loading="warGuessLoading">
              刷新数据
            </a-button>
          </div>
        ...
        <div class="modal-actions" style="margin-top: 20px; text-align: right">
          <a-button @click="showWarGuessModal = false">关闭</a-button>
        </div>
      </div>
    </a-modal>
    ...
    >
      <div class="settings-content">
        <!-- 创建新分组 -->
        <a-divider title-placement="left" style="margin: 0 0 16px 0">
          创建新分组
        </a-divider>
        <div style="margin-bottom: 24px">
          ...
          >
            ...
            <a-button type="primary" size="small" @click="createNewGroup">
              创建分组
            </a-button>
          </div>
          ...
          >
            ...
            >
              ...
              >
              <a-space size="small">
                <a-button size="tiny" @click="selectAllNewGroup">全选</a-button>
                <a-button size="tiny" @click="deselectAllNewGroup"
                  >全不选</a-button
                >
              </a-space>
            </div>
            <div style="max-height: 150px; overflow-y: auto">
              <a-checkbox-group v-model="newGroupSelectedTokens">
                <a-grid :cols="3" :x-gap="12" :y-gap="8">
                  <a-grid-item v-for="token in sortedTokens" :key="token.id">
                    <a-checkbox :value="token.id">{{ token.name }}</a-checkbox>
                  </a-grid-item>
          ...
          >
            ...
            >
              ...
              <div
                ...
                v-if="editingGroupId !== group.id"
              >
                <a-button size="small" @click="startEditGroup(group.id)">
                  编辑
                </a-button>
                <a-button
                  size="small"
        ...
        <!-- 关闭按钮 -->
        <div class="modal-actions" style="margin-top: 20px; text-align: right">
          <a-button @click="showGroupManageModal = false">关闭</a-button>
        </div>
      </div>
    </a-modal>
  </div>
...
import { storage } from "@/utils/crossPlatform";
...
const getStorageKey = (tokenId, suffix) => `${STORAGE_KEY_PREFIX}${tokenId}_${suffix}`;
...
const isWeekResetNeeded = (savedDate, currentDate) => {
  ...
  const isInResetWindow =
    (currentDay === 0 && currentHour >= 23) ||
    (currentDay === 1 && currentHour < 23);
...
const saveCardStatus = async (tokenId) => {
  try {
    const statusData = {
      saltJar: saltJar.value[tokenId],
      hangUp: hangUp.value[tokenId],
      dailyTask: dailyTask.value[tokenId],
      monthlyTask: monthlyTask.value[tokenId],
      legacyStatus: legacyStatus.value[tokenId],
      starChallengeTotalStars: starChallengeTotalStars.value[tokenId],
      arenaRank: arenaRank.value[tokenId],
      studyStatus: studyStatus.value[tokenId],
      towerData: towerData.value[tokenId],
      weirdTowerData: weirdTowerCardData.value[tokenId],
      towerInfo: towerInfo.value[tokenId],
      carStatus: carStatus.value[tokenId],
      savedAt: new Date().toISOString(),
    };
    const storageKey = getStorageKey(tokenId, "status");
...
const restoreCardStatus = async (tokenId) => {
  try {
    const storageKey = getStorageKey(tokenId, "status");
    console.log(`[恢复状态] Token: ${tokenId}, 尝试读取: ${storageKey}`);
    ...
    if (savedData.saltJar) {
      saltJar.value[tokenId] = { ...saltJar.value[tokenId], ...savedData.saltJar };
    }
    ...
    if (savedData.hangUp) {
      ...
      hangUp.value[tokenId] = { ...hangUp.value[tokenId], ...hangUpData };
    }

    if (savedData.dailyTask) dailyTask.value[tokenId] = { ...dailyTask.value[tokenId], ...savedData.dailyTask };
    if (savedData.monthlyTask) monthlyTask.value[tokenId] = { ...monthlyTask.value[tokenId], ...savedData.monthlyTask };
    if (savedData.legacyStatus) legacyStatus.value[tokenId] = { ...legacyStatus.value[tokenId], ...savedData.legacyStatus };
    if (savedData.starChallengeTotalStars !== undefined) starChallengeTotalStars.value[tokenId] = savedData.starChallengeTotalStars;
    if (savedData.arenaRank !== undefined) arenaRank.value[tokenId] = savedData.arenaRank;
    if (savedData.studyStatus) studyStatus.value[tokenId] = { ...studyStatus.value[tokenId], ...savedData.studyStatus };

    if (savedData.towerData) {
      const { isExpanded, ...towerRest } = savedData.towerData;
      towerData.value[tokenId] = { ...towerData.value[tokenId], ...towerRest };
    }

    if (savedData.weirdTowerData) {
      const { isExpanded, ...weirdTowerRest } = savedData.weirdTowerData;
      weirdTowerCardData.value[tokenId] = { ...weirdTowerCardData.value[tokenId], ...weirdTowerRest };
    }

    if (savedData.towerInfo) towerInfo.value[tokenId] = { ...towerInfo.value[tokenId], ...savedData.towerInfo };
    if (savedData.carStatus) carStatus.value[tokenId] = { ...carStatus.value[tokenId], ...savedData.carStatus };
...
const startStarChallenge = async (tokenId) => {
  const token = tokens.value.find((t) => t.id === tokenId);
  if (!token) {
  ...
  try {
    ...
    addLog({ message: `[${token.name}] 开始 十殿星级挑战，一键挑战`, type: "info" });
    message.info("星级挑战开始");
    ...
    for (let level = 1; level <= 8; level++) {
      if (shouldStop.value) {
        addLog({ message: "用户手动停止,挑战终止", type: "error" });
        break;
      }
    ...
    await loadStarChallengeData(tokenId);
  } catch (error) {
...
const loadStarChallengeData = async (tokenId) => {
  try {
    ...
    if (wsStatus === "connected") {
      try {
        ...
        if (nmextInfo) {
          ...
          if (starChallengeTotalStars.value[tokenId] !== totalStars) {
            starChallengeTotalStars.value[tokenId] = totalStars;
    ...
    if (!cachedData) {
      starChallengeTotalStars.value[tokenId] = 0;
      return;
    }

    const parsedCache = JSON.parse(cachedData);
    const tokenData = parsedCache[tokenId];
    if (!tokenData) {
      starChallengeTotalStars.value[tokenId] = 0;
      return;
    }
    ...
    if ((tokenData.weekStart || 0) === thisMonday) {
      starChallengeTotalStars.value[tokenId] = tokenData.totalStars || 0;
    } else {
      starChallengeTotalStars.value[tokenId] = 0;
    }
  } catch (error) {
...
const fetchCarStatus = async (tokenId) => {
  const dayOfWeek = new Date().getDay();
  if (!(dayOfWeek >= 1 && dayOfWeek <= 3)) {
    return;
  }
  try {
    carStatus.value[tokenId] = { ...carStatus.value[tokenId], isLoading: true };
    ...
    carStatus.value[tokenId] = { ...carStatus.value[tokenId], isLoading: false };
  }
};

const fetchArenaRank = async (tokenId) => {
  try {
    ...
    if (myRankEntry) {
      arenaRank.value[tokenId] = myRankEntry.rank || myRankEntry.info?.rank || 0;
    } else {
      arenaRank.value[tokenId] = 0;
    }
  } catch (error) {
...
const startArenaFight = async (tokenId) => {
  ...
  try {
    ...
    addLog({ message: `[${token.name}] 开始竞技场挑战 (${fightCount}/3次)`, type: "info" });

    for (let i = 0; i < fightCount && !shouldStop.value; i++) {
...
const claimLegacy = async (tokenId) => {
  try {
    const token = tokens.value.find((t) => t.id === tokenId);
    if (!token) {
      message.warning("Token不存在");
    ...
    if (result.reward && result.reward.length > 0) {
      ...
      legacyStatus.value[tokenId] = {
        ...legacyStatus.value[tokenId],
        quantity: (legacyStatus.value[tokenId]?.quantity || 0) + claimedAmount,
        lastClaimTime: new Date().toLocaleTimeString(),
        isAvailable: false,
      };
      addLog({
        time: new Date().toLocaleTimeString(),
        message: `成功领取功法残卷${claimedAmount}，当前共有${legacyStatus.value[tokenId].quantity}个`,
        type: "success",
      });
    }
...
const startBatch = async () => {
  ...
  wakeLockManager.request().catch((e) => {
    console.warn("WakeLock请求失败:", e);
  });
  ...
  wakeLockManager.release().catch((e) => {
    console.warn("WakeLock释放失败:", e);
  });
