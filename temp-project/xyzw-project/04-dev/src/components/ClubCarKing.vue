<template>
  <a-card class="club-car-king" embedded>
    <a-card>
      <template #avatar>
        <a-avatar
          :size="48"
      ...
      <template #description>
        <a-space size="small" style="margin-top: 4px;">
          ...
          <a-tag size="small" :type="hasFreeRefresh ? 'success' : 'default'">
            {{ hasFreeRefresh ? `有 ${freeCarsCount} 辆可免费刷新` : "无免费刷新" }}
          </a-tag>
          <a-tag size="small" type="warning">
            <template #icon>
              <Ticket />
            </template>
            剩余车票: {{ refreshTickets }}
    ...
    <div class="card-content" style="margin-top: 16px;">
      <div v-if="!isConnected" class="hint">
        <a-empty description="请先选择 Token 并建立连接" />
      </div>
      <div v-else-if="carList.length === 0 && !carLoading" class="hint">
        <a-empty description="暂无车辆数据" />
      </div>

      <a-grid x-gap="12" y-gap="12" cols="1 600:2 900:3 1200:4" item-responsive v-if="carList.length > 0">
        <a-col v-for="c in carList" :key="c.key">
          <a-card size="small" class="car-card-item" content-style="padding: 12px;">
            <div class="car-header">
              ...
              <div class="car-info">
                ...
                <div class="car-badges">
                  <a-tag size="tiny" :color="{ color: getGradeColor(c.color), textColor: '#fff' }">
                    {{ gradeLabel(c.color) }}
                  </a-tag>
                  <a-tag v-if="c.level != null" size="tiny">Lv.{{ c.level }}</a-tag>
                  <a-tag v-if="c.star != null" size="tiny">{{ c.star }}星</a-tag>
                </div>
              </div>
            </div>

            <div class="car-status" style="margin-top: 12px;">
              <a-grid cols="2" x-gap="8" y-gap="8">
                <a-col>
            ...
            <div class="car-actions" style="margin-top: 16px;">
              <a-grid cols="3" x-gap="8">
                ...
                <a-col>
                  ...
                  >
                    <template #icon><CarSport /></template>
  ...
  <!-- 护卫选择弹窗 -->
  <a-modal
    v-model:visible="helperDialogVisible"
    preset="card"
    title="选择护卫"
    style="width: 600px"
  >
    ...
    <template #footer>
      <a-space justify="end">
        <a-button @click="cancelHelper">取消</a-button>
        <a-button type="primary" @click="confirmHelper">确定</a-button>
      </a-space>
    </template>
  </a-modal>
</template>
...
import { IconDriveFile, IconRefresh, IconThunderbolt, IconArrowUp, IconUser, IconTags } from '@arco-design/web-vue/es/icon';
