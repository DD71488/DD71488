  ...
  <div class="legion-war-statistics-container">
    <div class="legion-war-statistics-card">
      <div v-if="!isAccessible" class="access-denied-container">
        <a-result status="403" title="暂未开放" description="该功能仅在特定时间开放">
          <template #footer>
      ...
      <div class="function-section">
        <div class="function-left">
          <a-radio-group v-model="viewMode" size="small">
            <a-radio value="legion">战队战况</a-radio>
            <a-radio value="individual">个人战况</a-radio>
            <a-radio value="all_individual">全部战况</a-radio>
          </a-radio-group>
          
          <div class="stat-item">
            <span class="stat-label">连接状态:</span>
            <a-tag :type="isConnected ? 'success' : 'error'">
              {{ isConnected ? "已连接" : "未连接" }}
            </a-tag>
          </div>
          <div class="stat-item">
            <span class="stat-label">战场数据:</span>
            <a-tag :type="battlefieldId ? 'success' : 'error'">{{ battlefieldId ? "已成功获取战场数据" : "未获取到战场数据" }}</a-tag>
          </div>
          <div class="stat-item">
            <span class="stat-label">更新时间:</span>
            <a-tag type="warning">{{ lastUpdateTime || "等待数据..." }}</a-tag>
          </div>
        ...
        >
          <template #empty>
            <div class="empty-state">
              ...
              <template v-else>
                <a-empty description="暂无战场数据，请手动刷新数据">
                  <template #icon>
...
import { IconExport, IconNotification, IconBarChart, IconRefresh, IconImage, IconDownload } from '@arco-design/web-vue/es/icon';
