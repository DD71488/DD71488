<template>
  <MyCard class="club-info" :statusClass="{ active: !!club }">
    ...
    <template #default>
      <div v-if="!club" class="empty-club">
        <a-empty description="暂无俱乐部" />
        <div class="actions">
          <a-button size="small" @click="refreshClub">刷新</a-button>
        </div>
      </div>
      <div v-else>
        <div class="toolbar">
          <a-space size="small">
            <!-- 申请列表按钮 -->
            <a-button v-if="canKick" size="small" @click="getApplyList">申请列表</a-button>
            <a-button size="small" @click="refreshClub">刷新</a-button>
          </a-space>
        </div>
              ...
              >
                ...
                <div class="apply-right">
                  <a-space size="small">
                    ...
                    <a-button size="tiny" @click="rejectApply(apply.roleId)">
                      拒绝
                    </a-button>
                  </a-space>
                </div>
        ...
        <a-tabs v-model="activeTab" type="line" animated>
          <a-tab-pane key="overview" tab="概览" display-directive="show:lazy">
            <div class="overview">
              <a-grid x-gap="12" y-gap="12" cols="2" item-responsive>
                <!-- 头部信息 -->
                <a-col span="2">
                  <a-card embedded content-style="padding: 16px;">
                    <a-card>
                      <template #avatar>
                        <a-avatar
                          :size="64"
                      ...
                      <template #description>
                        <a-space size="small" style="margin-top: 4px;">
                          <a-tag size="small" type="warning">ID: {{ club.id }}</a-tag>
                          <a-tag size="small" type="info">服务器: {{ club.serverId - 27 }}</a-tag>
                          <a-tag size="small" type="success">成员: {{ memberCount }}</a-tag>
                        </a-space>
                      </template>
                      <template #header-extra>
                        <a-button
                ...
                <!-- 统计数据 -->
                <a-col>
                  <a-card size="small" embedded style="height: 100%;">
                    <a-statistic label="战力">
                      <template #prefix>
                        <BarChart />
                      </template>
                ...
                <!-- Boss 击杀情况 -->
                <a-col span="2" v-if="clubOverview.unfoughtBosses && clubOverview.unfoughtBosses.length > 0">
                  <a-alert type="warning">
                    ...
                    <a-collapse arrow-placement="right" style="margin-top: 8px;">
                      <a-collapse-item title="展开查看遗漏Boss列表" name="1">
                        <a-space size="small" style="margin-top: 8px;">
                          <a-tag v-for="boss in clubOverview.unfoughtBosses" :key="boss" type="error" size="small">
                            {{ boss }}
                          </a-tag>
                        </a-space>
                      </a-collapse-item>
                ...
                <!-- 公告 -->
                <a-col span="2" v-if="club.announcement">
                  <a-card size="small" title="公告" embedded>
                    <template #header-extra>
                      <Megaphone />
                    </template>
                ...
                <!-- 会长 -->
                <a-col span="2" v-if="leader">
                  <a-card size="small" title="会长" embedded>
                    <template #header-extra>
                      <IconUser />
                    </template>
          ...
          <a-tab-pane key="members" tab="成员" display-directive="show:lazy">
  ...
  <!-- 玩家信息模态框 -->
  <a-modal
    v-model:visible="showPlayerInfoModal"
    preset="card"
    title="成员信息"
    :style="{ width: '800px' }"
  ...
  >
    ...
    <template #footer>
      <a-button @click="showPlayerInfoModal = false">关闭</a-button>
    </template>
  </a-modal>
...
import { IconCopy, IconRefresh, IconUserGroup, IconBarChart, IconFire, IconExclamationCircle, IconNotification, IconUser, IconSafe } from '@arco-design/web-vue/es/icon';
...
/** APK528 新增：统计红将数量 */
const countRedGenerals = (members) => {
  if (!Array.isArray(members)) return 0;
  return members.filter((m) => {
    const heroes = m.heroes || m.lineup || [];
    return heroes.some((h) => Number(h.color || h.quality || 0) >= 5);
  }).length;
};
</script>
