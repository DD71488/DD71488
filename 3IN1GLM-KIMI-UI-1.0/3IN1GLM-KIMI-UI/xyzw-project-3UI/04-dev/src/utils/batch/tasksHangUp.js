...
export function createTasksHangUp(deps) {
  ...
  /**
   * 一键完成日常任务（APK528 新增）
   * 自动执行：签到、领取挂机奖励、答题、开箱、领取日常/周常奖励
   */
  const runDailyTaskCompletion = async () => {
    isRunning.value = true;
    const taskPromises = selectedTokens.value.map(async (tokenId) => {
      if (shouldStop.value) return;
      const token = tokens.value.find((t) => t.id === tokenId);
      ...
      try {
        ...
        addLog({
          time: new Date().toLocaleTimeString(),
          message: `${token.name} 开始一键日常...`,
          type: "info",
        });

        const client = tokenStore.getWebSocketClient(tokenId);
        if (!client) {
          addLog({ time: new Date().toLocaleTimeString(), message: `${token.name} 连接失败`, type: "error" });
          return;
        }
        ...
        // 2. 领取挂机奖励
        try {
          await client.sendWithPromise("hangup_claimreward");
          results.push("挂机奖励");
        } catch (e) { /* 跳过 */ }

        // 3. 领取日常任务奖励
        try {
          await client.sendWithPromise("task_claimdailyreward");
          results.push("日常奖励");
        } catch (e) { /* 跳过 */ }

        // 4. 领取周常任务奖励
        try {
          await client.sendWithPromise("task_claimweeklyreward");
          results.push("周常奖励");
        } catch (e) { /* 跳过 */ }

        // 5. 开箱
        try {
          await client.sendWithPromise("item_openbox");
          results.push("开箱");
        } catch (e) { /* 跳过 */ }
        ...
        // 7. 领取卡片奖励
        try {
          await client.claimAllCardRewards();
          results.push("卡片奖励");
        } catch (e) { /* 跳过 */ }

        addLog({
          time: new Date().toLocaleTimeString(),
          message: `${token.name} 一键日常完成: ${results.join(", ")}`,
          type: "success",
        });
      } catch (error) {
        addLog({
          time: new Date().toLocaleTimeString(),
          message: `${token.name} 一键日常出错: ${error.message}`,
          type: "error",
        });
      } finally {
        tokenStore.closeWebSocketConnection(tokenId);
