...
| 构建验证 | 0% | 从未 `npm run build` |

### 1.3 阻断性 Bug

[main.js](04-dev/src/main.js) 缺少关键代码：

```javascript
// 缺失 1: Arco CSS 全局样式（所有 <a-xxx> 组件无样式渲染）
import "@arco-design/web-vue/dist/arco.css";

// 缺失 2: Arco Vue 插件注册（Message/Modal/Notification 等命令式 API 不可用）
...
| # | 内容 | 类型 | 估计 | 依赖 |
|---|------|------|------|------|
| P0-1 | main.js 修复（Arco CSS + 插件注册） | 阻断修复 | 0.5h | 无 |
| P0-2 | `npm install && npm run build` 验证 | 构建验证 | 0.5h | P0-1 |
