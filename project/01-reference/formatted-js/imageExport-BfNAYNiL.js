import "./index-BUwMeHKm.js";
const R = { FREE: 3, PAID: 1 },
  _ = {
  ...
  b = { WEEKLY: 4001, MONTHLY: 4002, PERMANENT: 4003 },
  G = 4,
  A = 3,
  O = 3,
  F = 3,
  ...
  C = (s) => {
    const t = (
      typeof s == "string" ? s : (s == null ? void 0 : s.message) || ""
    ).match(/\d{6,7}/);
    return t ? t[0] : "";
  },
  $ = (s, e = "") => {
  ...
  f = (s, e, t = null) =>
    e.split(".").reduce((a, r) => (a && a[r] !== void 0 ? a[r] : t), s),
  z = (s) =>
    s
      ? Array.isArray(s)
        ? s
        : s.rankList || s.roleList || s.targets || s.targetList || s.list || []
      : [],
  L = (s) => ({
...
class ne {
  ...
  loadSettings(e) {
    try {
      ...
      return (console.error("设置加载失败:", t), { ...I });
    }
  }
  isConnected() {
    var e, t;
    return (
      ((t =
        (e = this.tokenStore.wsConnections) == null
          ? void 0
          : e[this.tokenId]) == null
        ? void 0
        : t.status) === "connected"
  ...
  async ensureConnection(e = 0) {
    ...
    try {
      if (await this.tokenStore.attemptTokenRefresh(this.tokenId, !0)) {
        if ((await p(2e3), this.isConnected()))
          return (this.success("连接已恢复"), !0);
  ...
  async sendCommand(e, t = {}, a = {}) {
    ...
    try {
      ...
      const l = await this.tokenStore.sendMessageWithPromise(
        this.tokenId,
        e,
        t,
        n,
      ...
      return (
        await p(this.delaySettings.commandDelay),
        r && this.success(`${r} - 成功`),
        l
      );
    } catch (l) {
      if (U(l) && c < m)
        return (
          this.warn(`[连接错误] ${r}，重试 (${c + 1}/${m})`),
          await p(1e3),
          this.sendCommand(e, t, { ...a, retryCount: c + 1 })
        );
      if (o && N(l)) {
        const d = $(l, i);
        return (this.info(d), { success: !0, silent: !0 });
      }
      if (r) {
        const d = $(l, i);
        N(l) || this.error(`${r} - 失败: ${d}`);
      }
      throw l;
    }
  }
  ...
  async getCurrentFormation() {
    try {
      const e = await this.sendCommand(
        "presetteam_getinfo",
        {},
        { description: "获取阵容信息" },
      );
    ...
    if (this.originalFormation)
      try {
        (await this.sendCommand(
          ...
          { description: "恢复原阵容", context: "formation", timeout: 1e4 },
        ),
  ...
  buildBasicTasks(e, t, a) {
      ...
      n(4) ||
        (r.push(() =>
          this.sendCommandSafe(
            "hero_recruit",
            { recruitType: R.FREE, recruitNumber: 1 },
            { description: "免费招募" },
          ),
        ),
        this.settings.payRecruit &&
          r.push(() =>
            this.sendCommand(
              "hero_recruit",
              { recruitType: R.PAID, recruitNumber: 1 },
              { description: "付费招募" },
            ),
          )),
      !n(6) && i("buy:gold"))
    ...
    return (
      !n(5) &&
        ...
        r.push(async () => {
          try {
            const o = await this.getHangUpStatus({
                checkAddTime: !0,
            ...
            if (!(o.hasData && o.elapsedTime >= c)) {
              const l = o.elapsedTime || 0;
              (this.info(
                `挂机时间${this.formatTime(l)}，未达到${this.formatTime(c)}，跳过领取`,
              ),
                await this.sendCommand(
                  "role_getroleinfo",
                  {},
            ...
            if (
              (this.info(
                `挂机时间${this.formatTime(o.elapsedTime)}，满足领取条件，开始领取...`,
              ),
              await this.sendCommand(
                "system_mysharecallback",
                {},
                { description: "初始化挂机奖励", timeout: 5e3 },
              ),
              await this.sendCommand(
                "system_claimhangupreward",
                {},
                { description: "领取挂机奖励", timeout: 1e4 },
              ),
              o.needAddTime)
            ) {
      ...
      !n(7) &&
        ...
        r.push(async () => {
          (await this.tryOpenBox(_.DIAMOND.id, _.DIAMOND.name)) ||
            (await this.tryOpenBox(_.WOODEN.id, _.WOODEN.name));
        }),
      r
    );
  }
    ...
    return this.settings.arenaEnable
      ? [
          async () => {
            ...
            this.info("开始竞技场战斗流程");
            const t = await this.switchFormationIfNeeded(
            ...
            try {
              try {
                await this.sendCommand(
                  "arena_startarea",
                  {},
                  { description: "开始竞技场", timeout: 1e4, context: "arena" },
                );
              } catch (a) {
                if (C(a) === "200020") {
              ...
              for (let a = 1; a <= y.MAX_FIGHTS; a++)
                (await this.executeArenaFight(a),
                  a < y.MAX_FIGHTS && (await p(1e3)));
      ...
      try {
        (n > 0 && (this.warn(`战斗${e} - 重试中...`), await p(15e3)),
          await this.sendCommand(
            ...
            { targetId: r.targetId },
            { description: `竞技场战斗${e}`, timeout: 1e4 },
          ));
        return;
      } catch (i) {
        if (C(i) !== "200750" || n >= 1) {
  ...
  buildBossTasks(e, t) {
    ...
    if (n > 0) {
      let o = !1;
      a.push(async () => {
        o = await this.switchFormationIfNeeded(
  ...
  buildFixedRewardTasks() {
    ...
    const t = [
      ...
      { name: "免费礼包", cmd: "card_claimreward" },
      {
        ...
        params: { cardId: b.WEEKLY },
      },
      {
        name: "月卡礼包",
        cmd: "card_claimreward",
        params: { cardId: b.MONTHLY },
      },
      {
        name: "永久卡礼包",
        cmd: "card_claimreward",
        params: { cardId: b.PERMANENT },
      },
    ];
    return (
      this.settings.claimEmail &&
        t.push({ name: "邮件奖励", cmd: "mail_claimallattachment" }),
      ...
      if (v(t[`genie:daily:free:${n}`])) {
        ...
        a.push(() =>
          this.sendCommandSafe(
            "genie_sweep",
            { genieId: n },
            { description: `${i}灯神扫荡`, timeout: 5e3, context: "genie" },
          ),
        );
      }
    for (let n = 0; n < O; n++)
    ...
    return this.settings.blackMarketPurchase
      ? [
          async () => {
            var e, t, a, r, n;
            try {
              const i = await this.sendCommand(
                "store_purchase",
                { goodsId: 1 },
                { context: "blackMarket" },
              );
              ...
              try {
                (await this.sendCommand(
                  "store_buy",
                  { goodsId: _.BRONZE.id },
                  { description: "青铜宝箱(兜底)", timeout: 3e4 },
                ),
                  this.success("兜底购买成功"));
              } catch (o) {
                C(o) === "1100010"
                  ? this.info("青铜宝箱已购买过")
                  : this.error(`兜底购买失败: ${o.message}`);
              }
            }
          },
        ]
  ...
  async fetchRoleData() {
    try {
      ...
      if ((this.success("角色信息获取成功"), !(e != null && e.role)))
      ...
      if (U(e)) {
        if (
          ...
          !(await this.ensureConnection(1)))
        )
          throw e;
        const t = await this.tokenStore.sendGetRoleInfo(this.tokenId);
        if ((this.success("角色信息获取成功"), !(t != null && t.role)))
          throw new Error("角色数据不存在");
        return t.role;
      }
      throw e;
  ...
  async getHangUpStatus(e = {}) {
    const {
      checkAddTime: t = !1,
      thresholdSeconds: a = 3600,
      maxHangUpTime: r = 36e3,
    } = e;
    try {
      ...
      const o = Date.now() / 1e3,
        ...
        l = o - c,
        d = l <= m,
        u = d ? Math.floor(m - l) : 0,
        w = Math.floor(d ? l : m),
        k = m > 0 ? Math.min(100, Math.floor((w / m) * 100)) : 0;
      let g = !1,
        h = "";
      t &&
        (d
          ? u < 7200
            ? ((g = !0),
              (h = `剩余时间不足2小时(${this.formatTime(u)})，建议加钟`))
            : m < r && u < 14400
              ? ((g = !0),
                (h = `挂机时间${this.formatTime(m)}，剩余${this.formatTime(u)}，可以加钟`))
              : ((g = !1),
                (h =
                  m >= r
      ...
      const T = {
        ...
        addTimeMessage: h,
        message: d
          ? `挂机中：${this.formatTime(w)}/${this.formatTime(m)}`
          : "挂机已完成",
      };
      return (
        t
          ? this.info(`挂机状态: ${T.message} | ${h}`)
          : this.info(`挂机状态: ${T.message}`),
        T
      );
    } catch (n) {
      return (
  ...
  async run(e, t = {}, a = null) {
    ...
    if (
      ((this.tokenId = e),
      (this.callbacks = t),
      (this.settings = a || this.loadSettings(e) || { ...I }),
      this.info("检查连接状态..."),
    ...
    if (this.checkActivity(r)) {
      (u = (d = this.callbacks) == null ? void 0 : d.onProgress) == null ||
        u.call(d, 100);
      return;
    }
    (this.info("开始执行每日任务"),
    ...
    const n = ((w = r.dailyTask) == null ? void 0 : w.complete) ?? {},
      ...
      l = m.length;
    this.info(`共 ${l} 个任务待执行`);
    for (let E = 0; E < m.length; E++)
      try {
        (await m[E](),
          (g = (k = this.callbacks) == null ? void 0 : k.onProgress) == null ||
            g.call(k, Math.floor(((E + 1) / l) * 100)),
          await p(this.delaySettings.taskDelay));
      } catch {}
    ((T = (h = this.callbacks) == null ? void 0 : h.onProgress) == null ||
      T.call(h, 100),
...
const se = {
  ...
  J = [
    {
      name: "吴国",
      required: [106, 111],
      colorProps: { color: "#f5222d", textColor: "#fff" },
    },
    {
      name: "姜维",
      required: [114, 111],
      colorProps: { color: "#237804", textColor: "#fff" },
    },
    {
      name: "毒爆",
      required: [108, 112, 120],
      colorProps: { color: "#722ed1", textColor: "#fff" },
    },
    {
      name: "吕赵",
      required: [107, 116, 118],
      colorProps: { color: "#faad14", textColor: "#fff" },
    },
    {
      name: "三蜀",
      required: [104, 110, 118],
      colorProps: { color: "#389e0d", textColor: "#fff" },
    },
    {
      name: "典韦",
      required: [117, 113],
      colorProps: { color: "#40a9ff", textColor: "#fff" },
    },
    {
      name: "司马懿",
      required: [101, 113],
      colorProps: { color: "#0050b3", textColor: "#fff" },
    },
    {
      name: "关羽",
      required: [103],
      colorProps: { color: "#73d13d", textColor: "#fff" },
    },
    {
      name: "吕布",
      required: [107, 116],
      forbidden: [118],
      colorProps: { color: "#f06f44ff", textColor: "#fff" },
    },
    {
      name: "控制毒",
    ...
    return (
      s &&
        ...
        Object.values(s.heroes).forEach((t) => {
          ((e[t.artifactId] = {
            FishInfo: V[(t.artifactId + "").substring(0, 4)],
      ...
      s &&
        ...
        Object.values(s.pearlMap).forEach((t) => {
          e[t.artifactId] &&
            ...
            (e[t.artifactId].slotMap = Object.values(t.slotMap).map((a) =>
              Object.assign({}, a, Q[a.colorId]),
            ));
        }),
      e
    );
  ...
  he = { 1: "战士", 2: "法师", 3: "射手", 4: "刺客", 5: "辅助", 6: "肉盾" },
  ue = {
    101: "战士生命",
    102: "战士攻击力",
    103: "战士暴击",
    ...
    new Promise((t, a) => {
      try {
        s.toBlob
          ? s.toBlob((r) => {
              if (!r) {
                (console.error("Canvas转换Blob失败"), M(s, e).then(t).catch(a));
                return;
              }
          ...
          : M(s, e).then(t).catch(a);
      } catch (r) {
        (console.error("导出图片出错:", r), M(s, e).then(t).catch(a));
      }
    }),
  H = (s, e) =>
    new Promise((t, a) => {
    ...
    try {
      ...
      const a = new FileReader(),
        r = await new Promise((n, i) => {
          ((a.onloadend = () => n(a.result.split(",")[1])),
            (a.onerror = i),
            a.readAsDataURL(s));
        });
    ...
    try {
      return window.Capacitor !== void 0
        ? (console.log("[downloadFile] APK environment detected, using share"),
          await W(s, e))
        : (console.log(
            "[downloadFile] Web/EXE environment, using direct download",
          ),
