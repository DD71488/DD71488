...
> **⚠️ 2026-06-05 纠正说明**：
> 1. **[新增] 行号缺失**：`downloadInApk` 和 `downloadFile` 的行号列仅填写"新增"，缺少 formatted-js 中的精确行号。
> 2. **[修改] 引用基线行号**：[修改] 表格中的行号引用的是开源源码（imageExport.js）的行号，而非 formatted-js 中的行号。
> 3. **映射范围扩展**：对应的 `imageExport_mapping.md` 实际包含 DailyTaskRunner 类 30+ 方法映射、HeroList 全部数据定义映射、dreamConstants 配置映射，远超本文件标注的"2新增+3修改"范围。
