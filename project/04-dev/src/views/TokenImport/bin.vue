<template>
  <!-- 手动输入表单 -->
  <a-form :model="importForm" :label-placement="'top'" :size="'large'" :show-label="true">


    <a-form-item :label="'bin文件'" :show-label="true">
...
const handleDownload = (roleInfo: any) => {
  ...
  try {
    ...
    const fileName = `bin-${serverNum}服-${roleIndex}-${roleInfo.roleId}-${roleInfo.name || roleInfo.nickname}.bin`;
...
const addSelectedRole = async (roleInfo: any) => {
  ...
  try {
    ...
    const roleName = roleInfo.name || roleInfo.nickname || `角色_${roleInfo.roleId}`;
