<template>
  <div class="wx-qrcode-import">
    ...
    <div class="form-actions">
      <a-button type="primary" block @click="generateQRCode" :loading="isProcessing">
        <template #icon>
...
<IconRefresh />
    ...
    <!-- 角色命名格式配置 -->
    <a-form :model="importForm" label-placement="top" :show-label="true" style="margin-top: 16px;">
      <a-form-item label="角色命名格式" :show-label="true">
        <a-input v-model="importForm.nameTemplate" placeholder="{name}-{index}-{id}" />
        <template #feedback>
    ...
    <a-list>
      <a-list-item v-for="(role, index) in roleList" :key="index">
        <div style="display: flex; justify-content: space-between; align-items: center; width: 100%">
          ...
          <a-button type="error" size="small" @click="removeRole(index)">
            删除
          </a-button>
        </div>
      </a-list-item>
    ...
    <!-- 操作按钮 -->
    <div class="form-actions">
      <a-button type="primary" size="large" block :loading="isImporting" @click="handleImport">
        <template #icon>
...
<IconUpload />
      ...
      <a-button long @click="$emit('cancel')" :disabled="isProcessing">
        <template #icon>
...
import { IconScan, IconRefresh, IconClose, IconUpload } from '@arco-design/web-vue/es/icon';
