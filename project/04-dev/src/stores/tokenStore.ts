...
// 跨标签页连接协调（临时运行时状态，保留localStorage用于storage事件通信）
...
export async function initTokenStore() {
  ...
  try {
    // 1. 迁移localStorage旧数据到IndexedDB
    await migrateFromLocalStorageIfNeeded();

    // 2. 从IndexedDB恢复gameTokens
    const tokens = await getAllTokenList();
    gameTokens.value = tokens;

    // 3. 恢复selectedTokenId
    const savedId = await getKV("selectedTokenId");
    if (savedId) selectedTokenId.value = savedId;

    // 4. 恢复tokenGroups
    const groups = await getAllTokenGroups();
    tokenGroups.value = groups;

    tokenLogger.info(`TokenStore初始化完成，恢复${tokens.length}个Token`);
  } catch (e) {
    tokenLogger.error("TokenStore初始化失败:", e);
  }
}

// IndexedDB持久化辅助函数（fire-and-forget，不阻塞UI）
function persistTokenItem(tokenData: TokenData) {
  putTokenItem(tokenData).catch((e: any) =>
    tokenLogger.warn("持久化token失败:", e),
  );
}

function persistTokenDelete(id: string) {
  deleteTokenItem(id).catch((e: any) =>
    tokenLogger.warn("删除token持久化失败:", e),
  );
}

function persistTokenGroups() {
  putTokenGroups(tokenGroups.value).catch((e: any) =>
    tokenLogger.warn("持久化分组失败:", e),
  );
}

function persistSelectedTokenId() {
  setKV("selectedTokenId", selectedTokenId.value).catch((e: any) =>
    tokenLogger.warn("持久化selectedTokenId失败:", e),
  );
}
...
export const useTokenStore = defineStore("tokens", () => {
  ...
  const updateToken = (tokenId: string, updates: Partial<TokenData>) => {
    ...
    if (index !== -1) {
      ...
      persistTokenItem(gameTokens.value[index]);
      return true;
    }
    return false;
  };
  ...
  const removeToken = async (tokenId: string) => {
    ...
    // 删除IndexedDB中的token数据和ArrayBuffer
    persistTokenDelete(tokenId);
  ...
  const updateCrossTabConnectionState = (
    ...
    // 跨标签页通信使用localStorage（storage事件）
    localStorage.setItem(`ws_connection_${tokenId}`, JSON.stringify(state));
  ...
  const importTokens = (data: any) => {
    try {
      if (data.tokens && Array.isArray(data.tokens)) {
        ...
        // 持久化到IndexedDB
        clearTokenList().then(() => {
          data.tokens.forEach((t: TokenData) => persistTokenItem(t));
        });
        return {
          success: true,
  ...
  const clearAllTokens = async () => {
    ...
    // 清空IndexedDB（tokenList + ArrayBuffer）
    await Promise.all([clearTokenList(), clearAll()]);
  };
