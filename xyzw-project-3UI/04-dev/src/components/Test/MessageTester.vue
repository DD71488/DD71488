<template>
  <div class="message-tester">
    <a-card title="消息加解密测试" class="mb-4">
      <div class="space-y-4">
        <!-- 选择Token -->
        <div>
          <a-select
        ...
        <!-- 预设消息测试 -->
        <a-divider title-placement="left"> 预设消息测试 </a-divider>
        <div class="grid grid-cols-2 gap-2">
          <a-button :disabled="!canSendMessage" @click="sendHeartbeat">
            💗 发送心跳
          </a-button>
          <a-button :disabled="!canSendMessage" @click="sendGetRoleInfo">
            👤 获取角色信息
          </a-button>
          <a-button :disabled="!canSendMessage" @click="sendGetDataVersion">
            📦 获取数据版本
          </a-button>
          <a-button :disabled="!canSendMessage" @click="sendSignIn">
            📅 签到
          </a-button>
        </div>

        <!-- 自定义消息发送 -->
        <a-divider title-placement="left"> 自定义消息 </a-divider>
        <div class="space-y-2">
          <a-input
            v-model="customCmd"
        ...
        <a-divider title-placement="left">
          <div class="flex items-center justify-between w-full">
            ...
            <div class="flex items-center gap-2">
              ...
              >
                <svg class="mr-1"vg viewBox="0 0 24 24">
                    <path
                      fill="currentColor"
        ...
        <div class="message-history max-h-96 overflow-y-auto">
          ...
          >
            ...
            <div v-if="message.cmd" class="text-sm mb-2">
              <strong>命令:</strong>
              <a-tag size="small" :type="getCommandTagType(message.cmd)">{{
                message.cmd
              }}</a-tag>
            </div>
            ...
            <div class="mt-2">
              <a-collapse>
                ...
                >
                  <!-- 原始数据和解码数据的选项卡 -->
                  <a-tabs type="card" size="small" animated>
                    <a-tab-pane key="formatted" display-directive="show:lazy">
                      <template #tab>
                        <svg class="mr-1"vg viewBox="0 0 24 24">
                            <path
                              fill="currentColor"
                      ...
                      <div class="json-display-container">
                        <div class="json-header">
                          <a-space size="small">
                            <a-tag size="small" type="info">格式化</a-tag>
                            <a-button
                              size="tiny"
                              type="primary"
                              ghost
                    ...
                    <a-tab-pane key="raw" display-directive="show:lazy">
                      <template #tab>
                        <svg class="mr-1"vg viewBox="0 0 24 24">
                            <path
                              fill="currentColor"
                      ...
                      <div class="json-display-container">
                        <div class="json-header">
                          <a-space size="small">
                            <a-tag size="small" type="warning">原始</a-tag>
                            <a-button
                              size="tiny"
                              type="warning"
                              ghost
                    ...
                    <a-tab-pane key="compact" display-directive="show:lazy">
                      <template #tab>
                        <svg class="mr-1"vg viewBox="0 0 24 24">
                            <path
                              fill="currentColor"
                      ...
                      <div class="json-display-container">
                        <div class="json-header">
                          <a-space size="small">
                            <a-tag size="small" type="success">紧凑</a-tag>
                            <a-button
                              size="tiny"
                              type="success"
                              ghost
...
/* 选项卡内的预览区域 */
.arco-tabs .n-tab-pane {
  position: relative;
}

/* 复制按钮悬停效果 */
.arco-btn[title*="复制"]:hover {
