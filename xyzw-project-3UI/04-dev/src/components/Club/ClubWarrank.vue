        ...
        <div class="function-left">
          <div class="export-options">
            ...
            >
              ...
              <a-checkbox value="2">图片导出</a-checkbox>
            </a-checkbox-group>
          </div>
        </div>
...
<IconEdit />
      ...
      <div ref="exportDom" class="table-content">
          ...
          >
            <div class="table-cell rank">
              <div v-if="isEditMode" class="edit-rank">
                <a-input-number
                  v-model="manualRankings[member.id]"
                  size="small"
                  :min="1"
                  :max="20"
                  style="width: 70px"
            ...
            <div class="table-cell alliance">
              <div v-if="isEditMode" class="edit-alliance">
                <a-select
                  v-model="manualAlliances[member.id]"
        ...
        <div v-else-if="!loading1" class="empty-state">
          <a-empty description="暂无盐场匹配数据" size="large">
            <template #icon>
...
import { IconTrophy, IconRefresh, IconCopy, IconDown, IconUp, IconFile, IconEdit } from '@arco-design/web-vue/es/icon';
import {
  getLastSaturday,
  formatTimestamp,
  formatTimestamp1,
...
const handleRankBlur = (member) => {
  ...
  if (!newRank || newRank < 1 || newRank > 20) {
    ...
    if (newRank !== null && newRank !== undefined) {
      // 只有当用户输入了无效值时才提示，清空不提示（虽然 InputNumber min=1 通常不会清空）
