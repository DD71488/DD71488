  ...
  <div class="peach-info-card" ref="exportDom">
    <div class="toolbar">
      <div class="left">
        ...
        <a-date-picker v-model="queryDate" @change="fetchBattleRecordsByDate" valueFormat="YYYY/MM/DD"
          :disabled-date="disabledDate" :defaultValue="queryDate" format="YYYY/MM/DD" />
      </div>
      <div class="right">
        <a-button size="small" :disabled="!opponentMembers.length" @click="handleExportImage"
          class="action-btn export-btn" style="margin-right: 8px">
          <template #icon>
<IconCopy />
</template>导出图片
        </a-button>
        <a-button size="small" :disabled="loading" @click="fetchBattleRecordsByDate" class="refresh-btn">
          <template #icon>
...
<IconRefresh />
    ...
    <div class="header-section" v-if="battleInfo">
      <div class="club-vs-container">
        ...
        <div class="club-info own">
          <a-avatar round :size="80" :src="battleInfo.ownClub?.logo || '/icons/xiaoyugan.png'" class="club-logo" />
        ...
        <div class="club-info opponent">
          <a-avatar round :size="80" :src="battleInfo.opponentClub?.logo || '/icons/xiaoyugan.png'" class="club-logo" />
          <div class="club-details">
    ...
    <div v-else-if="opponentMembers.length > 0" class="members-table">
      ...
      <a-table :columns="columns" :data="opponentMembers" size="small" striped flex-height />
    </div>

    <!-- Empty State -->
    <div v-else class="empty-state">
      <a-empty description="暂无敌方数据" />
    </div>

    <!-- 玩家信息模态框 -->
    <a-modal v-model:visible="showPlayerInfoModal" preset="card" title="对手信息" :style="{ width: '800px' }"
      ...
      <div v-if="playerInfo" class="player-info-content">
        <div class="player-info-main">
          <a-avatar round :size="60" :src="playerInfo.headImg" class="player-avatar" />
          <div class="player-info-detail">
            <h3>
              {{ playerInfo.name }}
              <a-tag v-if="playerInfo.legacy > 0" :style="{
                color: '#fff',
        ...
        <div class="action-section">
          <div style="display: flex; align-items: center; gap: 8px; flex: 1">
            <div class="fight-count-container">
              ...
              <a-input id="fightCount" v-model="fightCount" type="number" placeholder="请输入切磋次数" min="1" max="100"
                :step="1" class="fight-count-input" size="small" @input="validateFightCount" />
              <div class="fight-count-hint">范围: 1-100</div>
            </div>
            <a-button type="tertiary" @click="showPlayerInfoModal = false" size="small" style="margin-right: 8px">
              关闭
            </a-button>
          </div>
          <a-button type="primary" @click="handleDuel" :disabled="!isFightCountValid">
            切磋
          </a-button>
        </div>
        ...
        <div v-if="fightProgress.visible" class="fight-progress">
          ...
          <a-progress type="line" :percentage="fightProgress.percentage" :show-indicator="false" :stroke-width="8"
            status="processing" />
        </div>
        ...
        <div v-if="fightResult.visible" class="fight-result">
          ...
          <div class="result-list">
            <div v-for="(battle, index) in fightResult.resultCount" :key="index"
              ...
              <div class="battle-header">
                ...
                <a-tag :type="battle.isWin ? 'success' : 'error'" size="small">
                  {{ battle.isWin ? "胜利" : "失败" }}
                </a-tag>
              </div>
              ...
              <div class="battle-details">
                <div class="battle-side left-side">
                  <a-avatar round :size="32" :src="battle.leftheadImg" class="side-avatar" />
                  <div class="side-info">
                ...
                <div class="battle-side right-side">
                  <a-avatar round :size="32" :src="battle.rightheadImg" class="side-avatar" />
                  <div class="side-info">
                    <span class="side-name">{{
                      battle.rightName || "未知"
                    }}</span>
          ...
          <div class="result-actions">
            <a-button type="primary" @click="resetFightResult">重新切磋</a-button>
            <a-button @click="fightResult.visible = false">关闭结果</a-button>
          </div>
        </div>
        ...
        <div class="player-heroes">
          ...
          <div class="hero-list" v-if="playerInfo.heroList && playerInfo.heroList.length > 0">
            <div v-for="(hero, index) in playerInfo.heroList" :key="hero.heroId || index" class="hero-item"
              ...
              <a-avatar round :size="40" :src="hero.heroAvate" style="cursor: pointer" />
              <div class="hero-info">
    ...
    <!-- 武将详情模态框 -->
    <a-modal v-model:visible="showHeroModal" class="hero-detail-modal" preset="card" title="武将详情"
      :style="{ width: '600px' }" :segmented="{ content: 'soft', footer: 'soft' }">
      <div v-if="heroModealTemp" class="hero-modal-content">
        <div class="hero-modal-header">
          <a-avatar round :size="80" :src="heroModealTemp.heroAvate" class="hero-modal-avatar" />
          <div class="hero-modal-basic">
            ...
            <div class="hero-modal-stats">
              ...
              <a-tag :type="heroModealTemp.HolyBeast ? 'success' : 'warning'">
        ...
        <div class="hero-modal-details">
          <a-descriptions label-placement="left" column="3" bordered>
            <a-descriptions-item label="战力">
            ...
            <a-descriptions-item label="开孔数">
              {{ heroModealTemp.hole }}
            </a-descriptions-item>
            <a-descriptions-item label="红孔数">
              {{ heroModealTemp.red }}
            </a-descriptions-item>
            <a-descriptions-item label="四圣状态">
              {{ heroModealTemp.HolyBeast ? "已激活" : "未激活" }}
            </a-descriptions-item>
            <a-descriptions-item label="四圣等级" v-if="heroModealTemp.HolyBeast">
              {{ heroModealTemp.HBlevel }}
            </a-descriptions-item>
            <a-descriptions-item label="鱼灵">
              {{
            ...
            <a-descriptions-item label="鱼珠技能">
              {{
            ...
            <a-descriptions-item label="鱼灵洗练">
      ...
      <template #footer>
        <a-button @click="showHeroModal = false">关闭</a-button>
      </template>
    </a-modal>
  </div>
</template>
...
import { IconRefresh, IconCopy } from '@arco-design/web-vue/es/icon';
...
.members-table {
  ...
  flex: 1;
  overflow: hidden;
  /* Use Table's scroll or auto here */
  display: flex;
  flex-direction: column;
}
