<template>
  <div ref="listRootRef">
    <a-card v-if="data && data.length > 0" :title="title" class="server-role-list-card">
    ...
    <div class="server-role-list server-role-list--mobile">
      ...
      >
        <div class="server-role-card__main">
          <div class="server-role-card__title">{{ row.name || row.nickname || "未命名" }}</div>
        ...
        <div class="server-role-card__actions">
          <a-button type="primary" size="small" block @click="emit('add', row)">
            添加
          </a-button>
          <a-button type="info" size="small" block @click="emit('download', row)">
            下载
          </a-button>
        </div>
      </div>
...
const filteredData = computed(() => {
  ...
  return props.data.filter((row: any) => {
    ...
    const name = (row.name || row.nickname || "").toLowerCase();
...
.server-role-card__actions .arco-btn {
  flex: 1;
}
