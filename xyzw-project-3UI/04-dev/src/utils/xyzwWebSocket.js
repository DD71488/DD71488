...
export class XyzwWebSocketClient {
  constructor({ url, utils, heartbeatMs = 2000 }) {
    ...
    this.maxReconnectAttempts = 5; // 最大重连次数（对齐APK的WsAgent）
  ...
  init() {
    ...
    this.socket.onopen = () => {
      wsLogger.info("连接成功");
      this.connected = true;
      this.reconnectAttempts = 0; // 连接成功，重置重连计数器
      // 启动心跳机制
  ...
  /** 重连（防重复连接版本，对齐APK的maxReconnectAttempts=5） */
  reconnect() {
    // 防止重复重连
    ...
    wsLogger.info(`开始WebSocket重连...(${this.reconnectAttempts}/${this.maxReconnectAttempts})`);
  ...
  claimBoxWeeklyTargetReward() {
    return this.sendWithPromise("item_batchclaimboxpointreward", { weekly: true });
  }

  /** 领取招募周礼物 */
  claimRecruitWeeklyGift() {
    return this.sendWithPromise("hero_recruit", { recruitType: 1, recruitNumber: 1, weeklyGift: true });
  }

  /** 领取市场周礼物 */
  claimMarketWeeklyGift() {
    return this.sendWithPromise("store_buy", { storeId: 1, weeklyGift: true });
  }

  /** 领取宝箱周礼物 */
  claimBoxWeeklyGift() {
    return this.sendWithPromise("item_openbox", { weeklyGift: true });
  }

  /** 领取宝箱周锤子奖励 */
  claimBoxWeeklyHammerReward() {
    return this.sendWithPromise("item_batchclaimboxpointreward", { hammer: true });
  }

  /** 领取周一礼物 */
  claimMondayGift() {
    return this.sendWithPromise("system_mysharecallback", { type: 3, isSkipShareCard: true });
  }

  /** 开金蛋 */
  openGoldenEgg() {
    return this.sendWithPromise("artifact_lottery", { type: 2, lotteryNumber: 1 });
  }

  /** 领取所有周礼物 */
  claimAllWeeklyGifts() {
    return Promise.allSettled([
      this.claimBoxWeeklyGift(),
      this.claimRecruitWeeklyGift(),
      this.claimMarketWeeklyGift(),
    ]);
  }

  /** 领取青铜宝箱 */
  claimBronzeBox() {
    return this.sendWithPromise("item_openbox", { itemId: 1, number: 1 });
  }

  /** 领取周卡奖励 */
  claimWeeklyCardReward() {
    return this.sendWithPromise("card_claimreward", { cardId: 4001 });
  }

  /** 领取月卡奖励 */
  claimMonthlyCardReward() {
    return this.sendWithPromise("card_claimreward", { cardId: 4002 });
  }

  /** 领取终身卡奖励 */
  claimLifetimeCardReward() {
    return this.sendWithPromise("card_claimreward", { cardId: 4003 });
  }

  /** 领取所有卡片奖励 */
  claimAllCardRewards() {
    return Promise.allSettled([
      this.claimWeeklyCardReward(),
      this.claimMonthlyCardReward(),
      this.claimLifetimeCardReward(),
    ]);
  }
