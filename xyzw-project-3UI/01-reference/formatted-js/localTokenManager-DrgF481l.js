const __vite__mapDeps = (
  ...
  d = m.f ||
    (m.f = [
      "assets/wsAgent-C4mLjQ4W.js",
      "assets/index-BUwMeHKm.js",
      "assets/index-DlxoWD7I.css",
      "assets/gameCommands-BYMl-9HL.js",
    ]),
) => i.map((i) => d[i]);
import { aF as z, r as A, p as _, an as D } from "./index-BUwMeHKm.js";
...
function J() {
  return new Promise((o, s) => {
    ...
    ((n.onupgradeneeded = (k) => {
      const r = n.result;
      (r.objectStoreNames.contains(h) ||
        r.createObjectStore(h, { keyPath: "key" }),
        r.objectStoreNames.contains(w) ||
          r.createObjectStore(w, { keyPath: "roleId" }));
    }),
...
async function N(o) {
  return T(
    ...
    (s) =>
      new Promise((n, k) => {
        const r = s.get(o);
        ((r.onsuccess = () => n(r.result ? r.result.value : void 0)),
          (r.onerror = () => k(r.error)));
      }),
  );
}
...
async function I() {
  try {
    ...
    return (
      console.warn("Token DB migration skipped:", o),
      { migrated: !1, error: o == null ? void 0 : o.message }
    );
  }
}
const oe = z("localToken", () => {
  const o = A(null),
    ...
    a = () => {
      ((o.value = null),
        Y().catch((e) => console.warn("清除用户Token失败:", e)));
    },
    S = (e, t) => {
      const c = {
        ...t,
      ...
      return (
        (s.value[e] = c),
        y(e, c).catch((u) => console.warn("保存游戏Token失败:", u)),
        c
      );
    },
    d = (e) => {
      ...
      return (
        t &&
          ...
          y(e, t).catch((c) => console.warn("更新游戏Token失败:", c))),
        t
      );
    },
    p = (e, t) => {
    ...
    $ = (e) => {
      (delete s.value[e],
        G(e).catch((t) => console.warn("删除游戏Token失败:", t)),
        n.value[e] && v(e));
    },
    j = () => {
    ...
    C = async (e, t, c = null) => {
      ...
      try {
        const { WsAgent: u } = await D(
            async () => {
              const { WsAgent: g } = await import("./wsAgent-C4mLjQ4W.js");
              return { WsAgent: g };
            },
          ...
          { gameCommands: l } = await D(
            async () => {
              const { gameCommands: g } =
                await import("./gameCommands-BYMl-9HL.js");
        ...
        ((f.onOpen = () => {
          ((n.value[e].status = "connected"),
            ...
            setTimeout(() => {
              (f.send(l.role_getroleinfo(0, 0, { roleId: e })),
                f.send(l.system_getdatabundlever()));
            }, 1e3));
        }),
          (f.onMessage = (g) => {
            g.cmd && R(e, g);
          ...
          (f.onError = (g) => {
            (console.error(`❌ WebSocket错误 [${e}]:`, g),
              n.value[e] &&
                ((n.value[e].status = "error"),
                (n.value[e].lastError = g.message)));
          }),
          (f.onClose = (g) => {
          ...
          (f.onReconnect = (g) => {
            n.value[e] &&
              ((n.value[e].status = "reconnecting"),
              (n.value[e].reconnectAttempt = g));
          }));
        const E =
          c ||
          u.buildUrl("wss://xxz-xyzw.hortorgames.com/agent", {
            p: i,
            e: "x",
            lang: "chinese",
          });
        ...
        return (
          ...
          n.value[e] &&
            ((n.value[e].status = "error"), (n.value[e].lastError = u.message)),
          null
        );
      }
    },
    ...
    v = (e) => {
      ...
      t &&
        (t.agent && typeof t.agent.close == "function"
          ? t.agent.close()
          : t.connection &&
            typeof t.connection.close == "function" &&
            t.connection.close(),
        delete n.value[e]);
    },
    B = (e) => {
    ...
    M = async (e, t, c = {}, u = 8e3) => {
      ...
      if (l.status !== "connected")
        throw new Error(`角色 ${e} 的WebSocket未连接`);
      try {
        const { gameCommands: i } = l;
        if (typeof i[t] == "function")
          return await l.agent.sendWithPromise({ cmd: t, body: c, timeout: u });
    ...
    L = (e) => {
      try {
        ...
        return (
          console.error("Token导入失败:", t),
          { success: !1, message: "导入失败：数据格式错误" }
        );
      }
    },
    O = () => {
      ...
      return (
        Object.entries(s.value).forEach(([l, i]) => {
          new Date(i.lastUsed || i.createdAt) > t
            ? (c[l] = i)
            : (u++, n.value[l] && v(l), G(l).catch(() => {}));
        }),
        (s.value = c),
        u
      );
