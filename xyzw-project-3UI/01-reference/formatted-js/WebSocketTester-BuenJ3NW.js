      ...
      const l = Y(),
        ...
        B = async () => {
          ...
          try {
            ...
            (g.createWebSocketConnection(n.value, e.token, e.wsUrl),
              G(),
              l.success("WebSocket连接已启动"));
          } catch (e) {
            (console.error("WebSocket连接失败:", e),
              l.error("WebSocket连接失败: " + e.message));
          } finally {
            setTimeout(D, 1e3);
          }
        },
          ...
          try {
            ...
            (b.value.trim() && (e = JSON.parse(b.value)),
              g.sendMessage(n.value, r.value, e)
                ? (h("sent", { command: r.value, params: e }),
                  l.success("命令发送成功"))
                : l.error("命令发送失败"));
          } catch (e) {
            (console.error("发送命令失败:", e),
              l.error("发送命令失败: " + e.message));
          } finally {
            N.value = !1;
          }
        },
        ...
        j = async () => {
          ...
          try {
            ...
            for (let o = 0; o < 5; o++) {
              const k = g
                .sendMessageWithPromise(n.value, r.value, {
                  ...e,
                  requestIndex: o + 1,
                })
                .then((x) => ({
