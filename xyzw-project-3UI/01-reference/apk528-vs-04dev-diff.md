...
> ⚠️ **此文档生成于 2026-06-05，部分差异已在后续开发中补全。** 经 2026-06-09 验证，以下项目已补全：
> - 32 个 gameCommands 缺失命令 → 已全部注册
> - 13 条 responseToCommandMap 缺失 → 已全部注册
> - errorCodeMap 7300232 → 已存在
> - 14 个 XyzwWebSocketClient 便捷方法 → 已全部实现
> - 月赛 typeLabel 映射（type 15-25）→ 已实现
> - **仍缺失**：`runDailyTaskCompletion` 函数、`shouldSendCar` 的 matchAll 参数、`isTodayAvailable` 比较方式
> 请以代码实际状态为准。
